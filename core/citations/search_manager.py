...
class SearchManager:
    ...
    def search(
        self, topic: str, min_sources: int = None, quality_threshold: float = None
    ) -> List[Source]:
        """
        ...
        if len(validated_sources) < self.config.min_sources:
            print(
                f"Warning: Only found {len(validated_sources)} valid sources out of {self.config.min_sources} required"
            )
    ...
    def _search_semantic_scholar(self, topic: str) -> List[Source]:
        ...
        try:
            headers = (
                {"x-api-key": self.api_keys["semantic_scholar"]}
