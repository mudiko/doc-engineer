...
Do NOT wrap your response in any headings or formats - start directly with the content."""
    ...
    def create_document_plan(
        self, title: str, num_sections: int = 5, target_length_words: Optional[int] = None
    ) -> DocumentPlan:
...
{{
  ...
  "total_estimated_length": totalWordCount
        ...
        try:
            ...
            total_length = plan_data.get(
                "total_estimated_length",
                sum(s.estimated_length for s in [introduction] + main_sections + [conclusion]),
            )
